...
-- Description: TBD
-- =============================================
CREATE VIEW [Partition].[PartitionRanges] AS
...
select pf.name as [partition_function],
  ...
  from	sys.partition_functions pf
		...
		left join sys.partition_schemes ps on ps.function_id = pf.function_id
		left join sys.destination_data_spaces dds on dds.partition_scheme_id = ps.data_space_id AND dds.destination_id = prv.boundary_id + cast(pf.boundary_value_on_right as int)
		left join sys.data_spaces fg on fg.data_space_id = dds.data_space_id
 union all
select pf.name as [partition_function],
       ps.name as [partition_scheme],
