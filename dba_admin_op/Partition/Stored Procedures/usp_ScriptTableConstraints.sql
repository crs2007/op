...
-- =============================================
CREATE PROCEDURE [Partition].[usp_ScriptTableConstraints]
	@TableName sysname,
	...
	@CheckConstrient bit = 1
AS
BEGIN
	SET NOCOUNT ON;
	SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;

	SELECT @Create = N'',@Drop = N'';
...
' + IIF(ix.is_primary_key=0,'CREATE ' + case when ix.is_unique = 1 then 'UNIQUE ' else '' END + ix.type_desc + ' INDEX ' + QUOTENAME(CONCAT(ix.name,IIF(@DestanationTableName IS NULL,N'',N'_Temp'))) COLLATE DATABASE_DEFAULT +' ON ' + ISNULL(DT.[DestanationTable],QUOTENAME(schema_name(t.schema_id)) COLLATE DATABASE_DEFAULT +'.'+ QUOTENAME(t.name) COLLATE DATABASE_DEFAULT) ,
		 N'ALTER TABLE ' + QUOTENAME(schema_name(t.schema_id)) COLLATE DATABASE_DEFAULT +'.'+ QUOTENAME(t.name) COLLATE DATABASE_DEFAULT+ ' ADD CONSTRAINT ' + CONCAT(ix.name,IIF(@DestanationTableName IS NULL,N'',N'_Temp'+ISNULL(@AdditionalText,''))) COLLATE DATABASE_DEFAULT +' PRIMARY KEY '+ ix.type_desc + N' ') + 
		 '(' + SUBSTRING(IC.IColumn,0,LEN(IC.IColumn)) COLLATE DATABASE_DEFAULT+') '
		  + IIF(INC.IColumn IS NOT NULL,'INCLUDE (' + SUBSTRING(INC.IColumn,0,LEN(INC.IColumn)) COLLATE DATABASE_DEFAULT+ ')',N'') + ' WITH (' 
		 ...
		 @Drop += IIF(ix.is_primary_key=0,N'DROP INDEX ' + QUOTENAME(CONCAT(ix.name,IIF(@DestanationTableName IS NULL,N'',N'_Temp'+ISNULL(@AdditionalText,'')))) COLLATE DATABASE_DEFAULT +' ON ' + ISNULL(DT.[DestanationTable],QUOTENAME(schema_name(t.schema_id)) COLLATE DATABASE_DEFAULT +'.'+ QUOTENAME(t.name) COLLATE DATABASE_DEFAULT)+ ';
',N'
...
'),@Drop += 'ALTER TABLE [' + s.name + '].[' + t.name + '] DROP CONSTRAINT [' + cc.name + '];
'
    FROM	sys.check_constraints cc
			INNER JOIN sys.tables t ON t.object_id = cc.parent_object_id
			INNER JOIN sys.schemas s ON s.schema_id = t.schema_id
	where	cc.parent_object_id = OBJECT_ID(@DestanationTableName);
	if @CheckConstrient = 1
	SELECT	@Create +=ISNULL(N'
ALTER TABLE [' + s.name + '].[' + t.name + '] WITH CHECK ADD CONSTRAINT [CK_' + CONVERT(nvarchar(36),NEWID()) + N'_Temp] ' + [Partition].[ufn_GetPartitionConstreint] (@TableName,@PartitionNumber) + N';
',N'')
	FROM	sys.tables t 
			INNER JOIN sys.schemas s ON s.schema_id = t.schema_id
	where	t.object_id = OBJECT_ID(@DestanationTableName);
END